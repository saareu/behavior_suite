...
class CorrectionEpisode:
    """Contiguous run of prepared frames that contain automatic corrections."""
...
class TrackingReviewSession:
    ...
    def current_correction_episode(self) -> CorrectionEpisode | None:
        """Return the correction episode containing the current frame, if any."""
    ...
    def next_correction_episode(self) -> CorrectionEpisode | None:
        """Jump to the start of the next correction episode after the current frame."""
    ...
    def previous_correction_episode(self) -> CorrectionEpisode | None:
        """Jump to the start of the previous correction episode.
    ...
    # Legacy aliases used by older call sites; episode navigation is authoritative.
...
def accept_tracking_result(session: TrackingReviewSession) -> AcceptanceResult:
    ...
    if meta.get("acceptance_state") == ACCEPTANCE_ACCEPTED:
...
def _index_pose_points(
    ...
    *,
    ...
    frame_values = pd.to_numeric(frame[frame_column], errors="coerce")
    x_values = pd.to_numeric(frame["x"], errors="coerce")
    y_values = pd.to_numeric(frame["y"], errors="coerce")
    track_values = frame["track"].tolist()
    node_values = frame["node"].tolist()
